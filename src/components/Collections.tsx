      ...
      {_collections.sort((v, p) => v.name.toUpperCase() > p.name.toUpperCase() ? 1 : -1).map((collection) => (
        <li className="flex items-center justify-between">
          <div className="flex items-center gap-x-4">
            <div className="p-2 border border-gray-300 rounded text-gray-700">{VariablesImage(collection.types)}</div>
            <div className="grid gap-y-1 text-xs">
              ...
              <p className="text-gray-700">{collection.count} Variables</p>
            </div>
          </div>
          <Button
            name="edit"
